...
public class QnADao {
	...
	public ArrayList<QnAVo> deleteQnAList(Connection conn, String memberid) {
		...
		try {
			String sql  = "DELETE FROM cs_qna WHERE qna_title = ?, qna_text =?, qna_image =?";
			pstmt = conn.prepareStatement(sql);

			pstmt.setString(1, qnavo.getQnaTitle());
			pstmt.setString(2, qnavo.getQnaText());
			pstmt.setString(3, qnavo.getQnaImage());
